...
class ProjectWrapper(PDIWrapper):
    ...
    def _load(self):
        ...
        for dataset, image in data:
            ...
            iid = image.id.val
            iname = image.name.val
            log.debug("Adding dataset:%d image:%s" % (did, iid))
...
class ParsingContext(object):
    ...
    def __init__(self, client, target_object, file=None, dictionary_data=None,fileid=None,
    ...
    def create_table(self):
        ...
        log.debug('Created new table OriginalFile:%d' % original_file.id.val)
        table.initialize(self.columns)
        return table

    def parse_dictionary(self):
        # Directly use a dictionary for preprocessing and parsing
        if self.dictionary_data:
            self.preprocess_from_dictionary(self.dictionary_data)
            return self.parse_from_dictionary_data(self.dictionary_data)
        else:
            raise ValueError("Dictionary data is required for parsing")

    def preprocess_from_dictionary(self, dictionary):
        headers = dictionary.get("headers")
        items = dictionary.get("items")

        header_row = [header["value"] for header in headers if header["value"] != ""]
        header_types = dictionary.get("types")

        if self.column_types is None and header_types:
            self.header_types = HeaderResolver.get_column_types(header_types)
        self.column_types = header_types

        log.debug('Header: %r' % header_row)
        log.debug('Column types: %r' % self.column_types)

        self.header_resolver = HeaderResolver(self.target_object, header_row, column_types=self.column_types)
        self.columns = self.header_resolver.create_columns()
        log.debug('Columns: %r' % self.columns)
        if len(self.columns) > MAX_COLUMN_COUNT:
            log.warning("Column count exceeds max column count")
        ...
        #print("items", items)
        self.preprocess_data_from_items(items)

    def parse_from_dictionary_data(self, dictionary):
        items = dictionary.get("items")
        table = self.create_table()
        try:
            self.populate_from_items(items, table, batch_size=1000)
            self.create_file_annotation(table)
            log.debug('Column widths: %r' % self.get_column_widths())
            log.debug('Columns: %r' % [(o.name, len(o.values)) for o in self.columns])
            return table.getOriginalFile().id.val
        finally:
            table.close()

    def populate_from_items(self, items, table, batch_size=1000):
        row_count = 0
        for item in items:
            print(item)
            row = [item[getattr(col, "name", None)] for col in self.columns if getattr(col, "name", None) != "Image" ]
            #print("#######ROW##########", row)
            if row:
    ...
    def preprocess_data_from_items(self, items):
        column_count = 0
        for column in self.columns:
            ...
            for column, original_value in row:
                ...
                try:
                    ...
                    if isinstance(value, str):
                        ...
                        #print("############column size 1##############", column.size, " ", value.encode('utf-8'))
    ...
    def post_process(self):
        ...
        for i in range(0, sz):
            if well_name_column is not None:
                ...
                log.debug('Missing well name column, skipping.')
            ...
            if image_name_column is not None and (
                ...
                #print("############column size 2##############",image_name_column.size, " ", iname)
                ...
                iid = image_column.values[i]
                log.debug("Checking image %s", iid)
                pid = None
                ...
                #print("############column size 3##############", image_name_column.size, " ", iname)
    ...
    def write_to_omero(self):
        ...
        for cma in cmas:
            ...
            if len(batch) < self.batch_size:
                ...
                #print("_save_annotation_and_links with batch size %d" %              (self.batch_size))
...
class DeleteMapAnnotationContext(_QueryContext):
    ...
    def populate(self):
        ...
        log.debug("Total MapAnnotationLinks in %s: %d",
        ...
        if self.attach and NSBULKANNOTATIONSCONFIG in nss:
            ...
            log.debug("Total FileAnnotations in %s: %d",
